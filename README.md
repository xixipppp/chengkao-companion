...
[![Release](https://img.shields.io/badge/release-v2.2.0-ffd166)](https://github.com/xixipppp/chengkao-companion/releases)
...
> 规划中的后续闭环（见 `专家会议纪要-模块联动与管家中枢化.md`）：②弱点注入小游戏池（注入 ≤ 40%、换情境不复刻）、③课堂→诊断→排课回流（只插未到时段 10 分钟块且可撤销）、④角色×科目×进度三位一体（好感来源改为「完成任务」）。
