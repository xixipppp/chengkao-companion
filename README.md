...
[![Release](https://img.shields.io/badge/release-v2.3.0-ffd166)](https://github.com/xixipppp/chengkao-companion/releases)
...
> 规划中的后续闭环（见 `专家会议纪要-模块联动与管家中枢化.md`）：④角色×科目×进度三位一体（复用 `MOD2MATE`，好感来源从「互动」改为「完成任务」，陪学任意科都涨好感、仅擅长科有专属增益以防偏科）。
...
### 线上全模块回归（UAT）
`tests/online_regression.js` 直接访问**线上生产环境**，逐条核对全部模块与需求项（加载、导航、版本号、世界树、错题清零、AI 讲师回溯、切页保存、语音、10 小游戏、动画课堂、学习管家、变式封顶缓存、文案治理、黄金考点、**P0/P1 模块联动**、运行期错误），共 **20 个模块 / 102 条断言**：
